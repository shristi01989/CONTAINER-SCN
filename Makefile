...
.PHONY: build
build:
	go build $(LDFLAGS) ./cmd/containerscn

.PHONY: protoc
protoc:
	docker build -t containerscn-protoc - < Dockerfile.protoc
	docker run --rm -it -v ${PWD}:/app -w /app containerscn-protoc make _$@
...
.PHONY: install
install:
	go install $(LDFLAGS) ./cmd/containerscn
...
.PHONY: label
label: $(GOBIN)/labeler
	labeler apply misc/triage/labels.yaml -r aquasecurity/containerscn -l 5
